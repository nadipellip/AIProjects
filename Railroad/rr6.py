    ...
    while openSet:
        ...
        if stop not in closedSet:
            ...
            for nbr in myGraph[stop]:
                ...
                drawLine(canvas, nodes[nbr][0], nodes[nbr][1], nodes[stop][0], nodes[stop][1], 'blue')
                circles[nbr] = drawCircle(canvas, nodes[nbr][0], nodes[nbr][1], 'yellow')
                if stop in circles:
                    canvas.delete(circles[stop])
                if nbr == enode:
...
def backtrack(dict, goal, cities, canvas, nodes):
    ...
    while key != "":
        ...
        newkey = dict[key][0]
        if newkey != "":
            drawLine(canvas, nodes[key][0], nodes[key][1], nodes[newkey][0], nodes[newkey][1], 'red')
        key = newkey
    return str(path)
...
def drawLine(canvas, y1, x1, y2, x2, col):
    ...
    canvas.create_line((x1) * 10 + 1500, (50 - y1) * 10 + 400, (x2) * 10 + 1500, (50 - y2) * 10 + 400, fill=col)
...
# l = sys.argv
# solve(l[1:])
solve(["Los","Angeles", "Washington", "DC"])
