...
def negamax(brd, tkn, depth):
    moves = posmoves(brd, tkn)
    enemy = posmoves(brd,1-tkn)
    if not(enemy or moves):
        return [brd.count(OPTIONS[tkn]) - brd.count(OPTIONS[1-tkn])]
    if not moves:
        best = negamax(brd, 1- tkn,depth+1) + [-1]
        return [best[0]*-1] + best[1:]
    best = [-len(brd)-1]
    for mv in moves:
        nm = negamax(playmove(brd,tkn,mv),1-tkn,depth+1) + [mv]
        if ((nm[0]) > best[0] and tkn == 1 - TURN) :
            best = [nm[0]*-1] + nm[1:]
            if depth == 0:
                print([best])
