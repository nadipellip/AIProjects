...
def setLookups(inputval):
    global HEIGHT, WIDTH, SIZE, BOARD, TURN, MOVES, POSDCT, CORNERCONNECTS, MINICACHE,POSMOVESCACHE, INTDIVIDE
    act = True
    MINICACHE = {}
    POSMOVESCACHE = {}
    HEIGHT = 8
        ...
        if brd[i] == OPTIONS[tkn]:
            for d in range(8):  # 8 directions
                for n in range(WIDTH):
                    ...
                    if pos < 0 or pos >= SIZE or ((d == 0 or d == 4) and (INTDIVIDE[pos]) != (INTDIVIDE[i])) or ((d == 1 or d == 3 or d == 5 or d == 7) and abs((INTDIVIDE[pos]) - (INTDIVIDE[i])) != (n + 1)):
                        break
                    elif brd[pos] == OPTIONS[tkn]:
                        break
