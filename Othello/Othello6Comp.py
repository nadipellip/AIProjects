...
class Strategy():
    ...
    def best_strategy(self, board, player, best_move, running):
        ...
        if player == "@":
            player = "x"
        setLookups([board,player])
        choices = posmoves(BOARD,TURN)
        if choices:
            best_move.value = min(choices) + 11 + 2 * (min(choices) // 8)
            mv = choosePos(BOARD,TURN,choices)
            if mv:
                best_move.value = mv + 11 + 2 * (mv // 8)
        if BOARD.count(".") < 11:
            mm = minimax(BOARD,TURN,0,-65,65)
            best_move.value = mm[-1] + 11 + 2 * (mm[-1] // 8)


global HEIGHT, WIDTH, SIZE, BOARD, TURN, MOVES, POSDCT,MINICACHE,POSMOVESCACHE,OTHELLOFOUR
OPTIONS = ("X", "O")
CORNERS = {0, 7, 56, 63}
...
def minimax(brd, tkn,depth,alpha,beta):
    ...
    if (brd,tkn,alpha,beta) in MINICACHE:
        return MINICACHE[(brd,tkn,alpha,beta)]
    if not moves:
        ...
        if not(moves or enemy):
            ...
            MINICACHE[(brd, tkn, alpha, beta)] = bestValue
            return bestValue
    else:
        if tkn == TURN:
            bestValue = [-len(brd)-1]
